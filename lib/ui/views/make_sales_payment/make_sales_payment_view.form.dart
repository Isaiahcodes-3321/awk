...
final Map<String, TextEditingController>
    _MakeSalesPaymentViewTextEditingControllers = {};

final Map<String, FocusNode> _MakeSalesPaymentViewFocusNodes = {};

final Map<String, String? Function(String?)?>
    _MakeSalesPaymentViewTextValidations = {
  PaymentDescriptionValueKey: null,
  PaymentTransactionDateValueKey: null,
};

mixin $MakeSalesPaymentView {
  ...
  TextEditingController _getFormTextEditingController(
    ...
    if (_MakeSalesPaymentViewTextEditingControllers.containsKey(key)) {
      return _MakeSalesPaymentViewTextEditingControllers[key]!;
    }

    _MakeSalesPaymentViewTextEditingControllers[key] =
        TextEditingController(text: initialValue);
    return _MakeSalesPaymentViewTextEditingControllers[key]!;
  }

  FocusNode _getFormFocusNode(String key) {
    if (_MakeSalesPaymentViewFocusNodes.containsKey(key)) {
      return _MakeSalesPaymentViewFocusNodes[key]!;
    }
    _MakeSalesPaymentViewFocusNodes[key] = FocusNode();
    return _MakeSalesPaymentViewFocusNodes[key]!;
  }
  ...
  void disposeForm() {
    ...
    for (var controller in _MakeSalesPaymentViewTextEditingControllers.values) {
      controller.dispose();
    }
    for (var focusNode in _MakeSalesPaymentViewFocusNodes.values) {
      focusNode.dispose();
    }

    _MakeSalesPaymentViewTextEditingControllers.clear();
    _MakeSalesPaymentViewFocusNodes.clear();
  }
}
...
extension ValueProperties on FormStateHelper {
  ...
  set paymentDescriptionValue(String? value) {
    ...
    if (_MakeSalesPaymentViewTextEditingControllers.containsKey(
        PaymentDescriptionValueKey)) {
      _MakeSalesPaymentViewTextEditingControllers[PaymentDescriptionValueKey]
          ?.text = value ?? '';
    }
  }
...
String? getValidationMessage(String key) {
  final validatorForKey = _MakeSalesPaymentViewTextValidations[key];
  if (validatorForKey == null) return null;

  String? validationMessageForKey = validatorForKey(
    _MakeSalesPaymentViewTextEditingControllers[key]!.text,
  );
