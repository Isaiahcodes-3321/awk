...
class MarkPurchaseItemAsReceivedViewModel extends FormViewModel {
  ...
  final List<GlobalKey<FormState>>? purchaseItemFormKeys = [];
  ...
  MarkPurchaseItemAsReceivedViewModel({required this.purchase}) {
// Initialize controllers based on the number of purchase items
    for (int i = 0; i < purchase.purchaseItems.length; i++) {
      purchaseItemFormKeys?.add(GlobalKey<FormState>());
  ...
  Future<PurchaseStatusResult> markPurchaseItemAsRecieved(
      String purchaseItemId, int index, BuildContext context) async {
    ...
    final PurchaseStatusResult markItemSuccessful =
    ...
    if (markItemSuccessful.isCompleted) {
      ...
      purchase.purchaseStatusId = markItemSuccessful.purchaseStatus;
    } else {
